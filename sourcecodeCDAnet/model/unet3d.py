...
class inceptionRes3D(nn.Module):
    ...
    def __init__(self, in_channels, neck_channels, out_channels, final_relu=True):
        ...
        """
        super(inceptionRes3D, self).__init__()
        self.in_channels = in_channels
        self.neck_channels = neck_channels
        self.out_channels = out_channels


        # self.conv_a1 = nn.Conv3d(in_channels, int(neck_channels/2.0), kernel_size=1, stride=1)        
        # self.conv_b1 = nn.Conv3d(in_channels, int(neck_channels/2.0), kernel_size=3, stride=1, padding=1)
        # self.conv_c1 = nn.Conv3d(in_channels, int(neck_channels/2.0), kernel_size=5, stride=1, padding=2)

        # self.conv2 = nn.Conv3d(neck_channels + in_channels, factor_growth*neck_channels, kernel_size=(3, 5, 5), stride=1, padding=(1, 2, 2))

        self.conv_a1 = nn.Conv3d(in_channels, int(neck_channels/2.0), kernel_size=1, stride=1)        
        self.conv_b1 = nn.Conv3d(in_channels, int(neck_channels/2.0), kernel_size=(3, 5, 5), stride=1, padding=(1, 2, 2))
        self.conv_c1 = nn.Conv3d(in_channels, int(neck_channels/2.0), kernel_size=(5, 9, 9), stride=1, padding=(2, 4, 4))

        self.bn1 = nn.BatchNorm3d(num_features=int(neck_channels/2.0))
        self.bn2 = nn.BatchNorm3d(num_features=int(neck_channels/2.0))
        self.bn3 = nn.BatchNorm3d(num_features=int(neck_channels/2.0))

        self.conv_2  = nn.Conv3d(3*int(neck_channels/2.0), out_channels, kernel_size=1, stride=1)
        ...
        # OLD VERSION:
        """
        # self.conva1 = nn.Conv3d(in_channels, neck_channels, kernel_size=1, stride=1)
        # self.bna1 = nn.BatchNorm3d(num_features=neck_channels)
        # self.conva2 = nn.Conv3d(neck_channels, torch.floor(out_channels/3.) + torch.remainder(neck_channels, 3.), kernel_size=1, stride=1)
        # self.bna2 = nn.BatchNorm3d(num_features=torch.floor(out_channels/3.) + torch.remainder(neck_channels, 3.))
        
        # self.convb1 = nn.Conv3d(neck_channels, torch.floor(out_channels/3.), kernel_size=3, stride=1, padding=1)
        # self.bnb1 = nn.BatchNorm3d(num_features=torch.floor(out_channels/3.))
        # # self.convb2 = nn.Conv3d(neck_channels, torch.floor(out_channels/3.), kernel_size=1, stride=1)

        # self.convc1 = nn.Conv3d(neck_channels, torch.floor(out_channels/3.), kernel_size=5, stride=1, padding=2)
        # self.bnc1 = nn.BatchNorm3d(num_features=torch.floor(out_channels/3.))
        # # self.convc2 = nn.Conv3d(neck_channels, torch.floor(out_channels/3.), kernel_size=1, stride=1)
    ...
    def forward(self, x):  # pylint:
        ...
        branch0 = self.conv_a1(x)
        branch0 = self.bn1(branch0)
        branch0 = F.relu(branch0)

        branch1 = self.conv_b1(x)
        branch1 = self.bn2(branch1)
        branch1 = F.relu(branch1)

        branch2 = self.conv_c1(x)
        branch2 = self.bn3(branch2)
        branch2 = F.relu(branch2)

        x = torch.cat((branch0, branch1, branch2), dim=1)
        x = self.conv_2(x)
...
"""
DONE: NEEDS TESTING AND DEBUGGING...
Don't forget to compare the number of parameters...
"""
class denseBlock3D(nn.Module):
    """ 3D Convolutional Dense Block. Keeps the same dimensions, channels increase...
    """
    def __init__(self, in_channels, neck_channels, out_channels, final_relu=True):
        """
        Initialization.
        ...
        self.conv1 = nn.Conv3d(in_channels, neck_channels, kernel_size=1, stride=1)
        self.conv2 = nn.Conv3d(neck_channels + in_channels, factor_growth*neck_channels, kernel_size=(3, 5, 5), stride=1, padding=(1, 2, 2))
        self.conv3 = nn.Conv3d(in_channels + (factor_growth+1)*neck_channels, out_channels, kernel_size=1, stride=1)
        
        self.bn1 = nn.BatchNorm3d(num_features=neck_channels)
        self.bn2 = nn.BatchNorm3d(num_features=factor_growth*neck_channels)
        self.bn3 = nn.BatchNorm3d(num_features=out_channels)
    ...
    def forward(self, x):  # pylint:
        ...
        branch2 = self.conv2(torch.cat((x, branch1), dim=1))
        branch2 = self.bn2(branch2)
        branch2 = F.relu(branch2)

        branch3 = self.conv3(torch.cat((x, branch1, branch2), dim=1))
        branch3 = self.bn3(branch3)
        branch3 = F.relu(branch3)

        branch3 += self.shortcut(identity)
        if self.final_relu:
            branch3 = F.relu(branch3)
...
class ResBlock3D(nn.Module):
    ...
    def __init__(self, in_channels, neck_channels, out_channels, final_relu=True):
        ...
        self.out_channels = out_channels
        self.conv1 = nn.Conv3d(in_channels, neck_channels, kernel_size=1, stride=1)
        self.conv2 = nn.Conv3d(neck_channels, neck_channels, kernel_size=5, stride=1, padding=2)
        self.conv3 = nn.Conv3d(neck_channels, out_channels, kernel_size=1, stride=1)
        self.bn1 = nn.BatchNorm3d(num_features=neck_channels)
        self.bn2 = nn.BatchNorm3d(num_features=neck_channels)
        self.bn3 = nn.BatchNorm3d(num_features=out_channels)
...
class UNet3d(nn.Module):  # pylint: disable=too-many-instance-attributes
    ...
    def _create_layers(self):
        ...
        #                                                              nfeat_up_out)]
        """DENSE BLOCK 3D"""
        # self.conv_in = denseBlock3D(self.in_features, self.nf, self.nf)
        # self.conv_out = denseBlock3D(nfeat_down_in[0]*2, nfeat_down_in[0]*2, self.out_features,
        #                            final_relu=False)
        # self.conv_mid = denseBlock3D(nfeat_down_out[-1], nfeat_down_out[-2], nfeat_down_out[-2])
        # self.down_modules = [denseBlock3D(n_in, int(n/2), n) for n_in, n in zip(nfeat_down_in,
        #                                                                       nfeat_down_out)]
        # self.up_modules = [denseBlock3D(n_in, n, n) for n_in, n in zip(nfeat_up_in,
        #                                                              nfeat_up_out)]
        """INCEPTION BLOCK 3D"""
        self.conv_in = inceptionRes3D(self.in_features, self.nf, self.nf)
        self.conv_out = inceptionRes3D(nfeat_down_in[0]*2, nfeat_down_in[0]*2, self.out_features,
                                   final_relu=False)
        self.conv_mid = inceptionRes3D(nfeat_down_out[-1], nfeat_down_out[-2], nfeat_down_out[-2])
        self.down_modules = [inceptionRes3D(n_in, int(n/2), n) for n_in, n in zip(nfeat_down_in,
                                                                              nfeat_down_out)]
        self.up_modules = [inceptionRes3D(n_in, n, n) for n_in, n in zip(nfeat_up_in,
                                                                     nfeat_up_out)]
...
#             self.exp_modules = self.exp_modules + [ResBlock3D(self.nf, self.nf, self.nf) for _ in range(n_exp-1)]
            ...
            """DENSE BLOCK"""
            # self.exp_modules = [denseBlock3D(2*self.nf, 2*self.nf, 2*self.nf) for _ in range(n_exp)]
            """INCEPTION BLOCK"""
            self.exp_modules = [inceptionRes3D(2*self.nf, 2*self.nf, 2*self.nf) for _ in range(n_exp)]
    ...
    def _get_pool_kernel_size(prev_layer_dims):
        if np.all(prev_layer_dims == np.min(prev_layer_dims)):
            next_layer_dims = (prev_layer_dims/2).astype(int)
