...
class RB2DataLoader(Dataset):
    ...
    def __init__(self, data_dir, data_filename,
        ...
        # shape: (4, 200, 512, 128)
        self.data = RB2DataLoader._read_data_files(data_dir, data_filename)
        self.data = self.data[:,:,::1,...] # subsample frames
        # npdata = np.load(os.path.join(self.data_dir, self.data_filename))                       # LOADS THE DATASET FROM ONE SINGLE FILE
        # self.data = np.stack([npdata['p'], npdata['b'], npdata['u'], npdata['w']], axis=0)      # CONCATENATES TO HAVE PROPER VARIABLES
        # self.data = self.data.astype(np.float32)                                                # CONVERTS TO TYPE FLOAT32
        # self.data = self.data.transpose(0, 1, 3, 2)  # [c, t, z, x]                             # TRANSPOSES THE DIMENSIONS OF THE DATA TO [CHANNEL, TIME, Z-POSITION, X-POSITION]
        ...
        # normalize the dimensions for chopping
        ...
        self.rand_grid = np.stack(np.meshgrid(self.nf_start_range,
                                              self.nt_start_range,
                                              self.nz_start_range,
                                              self.nx_start_range, indexing='ij'), axis=-1)         # THIS STACKS TIME, Z-POSITION, X-POSITION TO GENERATE THE CUBE
        ...
        """
        Here I want to test a faster evaluation...
        """
        # self.grid_point_coord = np.stack(np.meshgrid(np.linspace(0, 1, self.nt_hres),
        #                                              np.linspace(0, 1, self.nz_hres),
        #                                              np.linspace(0, 1, self.nx_hres), indexing='ij'), axis=-1).astype(np.float32)
        # self.grid_point_coord = torch.from_numpy(self.grid_point_coord)
    ...
    def __getitem__(self, idx):
        ...
        space_time_crop_hres = self.data[f_id, :,
                                         ...
                                         x_id:x_id+self.nx_hres]  # [f, c, t, z, x]
        ...
        point_coord = np.random.choice(np.linspace(0., 1., 2000), (self.n_samp_pts_per_crop, 3)) * (self.scale_hres - 1)
        # point_coord = np.random.choice(np.linspace(0., 1., 500), (self.n_samp_pts_per_crop, 3)) * (self.scale_hres - 1)
        # np.random.rand(self.n_samp_pts_per_crop, 3) * (self.scale_hres - 1)
    ...
    def _read_data_files(data_folder, data_filenames):
        ...
        for data_filename in data_filenames:
            ...
            print("\n")
            print("loading data:")
            print(os.path.join(data_folder, data_filename))

            npdata_1 = np.load(os.path.join(data_folder, data_filename))
            print(npdata_1)
            print("\n")
            data_1 = np.stack([npdata_1['p'], npdata_1['b'], npdata_1['u'], npdata_1['w']], axis=0)
            print(os.path.exists(os.path.join(data_folder, data_filename)))
            all_data_1 = []
            all_data_1.append(data_1.transpose(0, 1, 3, 2))

            npdata = np.load(os.path.join(data_folder, data_filename))
            data = np.stack([npdata['p'], npdata['b'], npdata['u'], npdata['w']], axis=0)
            all_data.append(data.transpose(0, 1, 3, 2))  # [c, t, z, x]

        all_data = np.stack(all_data, axis=0).astype(np.float32) # [f, c, t, z, x] f=len(data_filenames)
        return all_data
...
#     plt.show()
    ...
    # def __getitem__(self, idx):
    #     """Loads the pair of clips, high res and low res, corresponding to idx. 

    #     Args:
    #       idx: int, index of the crop to return. must be smaller than len(self).

    #     Returns:
    #       space_time_crop_hres: array of shape [4, nt, nz, nx], where 4 are the phys channels pbuw.
    #       space_time_crop_lres: array of shape [4, nt, nz, nx], where 4 are the phys channels pbuw.
    #       grid_point_coord: array of shape [nt, nz, nx, 3], where 3 are the the t-z-x coordinates normalized between [0,1].
    #     """
    #     f_id, t_id, z_id, x_id = self.rand_start_id[idx]
    #     space_time_crop_hres = self.data[f_id, :,
    #                                      t_id:t_id+self.nt_hres,
    ...
    #     space_time_crop_lres = self.data[f_id, 
    #                                     :,
    #                                     t_id:t_id+self.nt_hres:self.downsamp_t,
    #                                     z_id:z_id+self.nz_hres:self.downsamp_xz,
    #                                     x_id:x_id+self.nx_hres:self.downsamp_xz]  # [f, c, t, z, x]
                                    
    #     if self.normalize_output:
    #         space_time_crop_hres = self.normalize_grid(space_time_crop_hres)
    #         space_time_crop_lres = self.normalize_grid(space_time_crop_lres)

    #     return space_time_crop_hres, space_time_crop_lres, self.grid_point_coord
