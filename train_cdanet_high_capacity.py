#!/usr/bin/env python3
"""
High-capacity CDAnet training script for powerful GPUs
Optimized for full GPU utilization while maintaining training stability
"""
...
# Set memory optimization flags for high-capacity training
...
def main():
    """High-capacity main function for powerful GPUs"""
    args = parse_args()

    # Override for high-capacity training
    print("🚀 Applying HIGH-CAPACITY optimizations for powerful GPU...")
    args.batch_size = 8           # Increased batch size
    args.n_samp_pts_per_crop = 768  # Balanced: not too high to avoid NaN, not too low
    args.nx = 96                  # Increased spatial resolution
    args.nz = 48                  # Increased spatial resolution
    args.nt = 12                  # Increased temporal resolution

    # Stability optimizations for NaN prevention
    args.lr = 0.01               # Reduced learning rate to prevent gradient explosion
    args.alpha_pde = 0.001       # Reduced PDE weight to prevent instability
    args.clip_grad = 0.5         # Aggressive gradient clipping

    print(f"High-capacity settings:")
    ...
    print(f"  PDE weight: {args.alpha_pde} (reduced for stability)")
    ...
    if args.device == 'auto':
        if torch.cuda.is_available():
            ...
            print("🚀 Using CUDA GPU with high-capacity optimization")
            # Clear CUDA cache
    ...
    args.output_folder = './checkpoints_high_capacity'
    ...
    # Create datasets with increased capacity
    ...
    # Create data loaders with higher capacity
    ...
    print("Starting high-capacity training with stability enhancements...\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, data_tensors in enumerate(train_loader):
            ...
            try:
                ...
                # Compute predictions and residues with chunking for stability
                chunk_size = 128  # Balanced chunk size
                total_reg_loss = 0.0
                total_pde_loss = 0.0
                ...
                for chunk_idx in range(n_chunks):
                    ...
                    # PDE loss (compute less frequently for stability)
                ...
                if "out of memory" in str(e).lower():
                    print(f"⚠️  OOM at batch {batch_idx}. Reducing batch for this iteration...")
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                    continue
                else:
        ...
        if epoch % 5 == 0:
            checkpoint = {
                ...
                'train_loss': avg_train_loss if valid_batches > 0 else float('inf'),
            }
            torch.save(checkpoint, os.path.join(args.output_folder, f'checkpoint_epoch_{epoch:03d}.pth'))
            print(f"  Checkpoint saved at epoch {epoch}")

    print("\n" + "=" * 60)
    print("High-capacity training completed!")
